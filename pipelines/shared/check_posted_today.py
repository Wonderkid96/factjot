...
def main() -> int:
    ...
    if kind == "reel":
        ...
        # list_posts.jsonl had no writer after ship_list_post.py was deleted in
        # Phase G.3. The autonomous list path (ship_carousel_post.py --type list)
        # writes to posted.jsonl with claim prefixed "list:". Read that only.
        ledger = Path("insta-brain/data/posted.jsonl")
        match = lambda row: (
            row.get("published_at", "").startswith(today)
